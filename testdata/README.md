...
1. **Frozen.** The vectors are a point-in-time capture of the C engine's output, committed once, loaded by the Go tests. Why: the C source contains no test vectors — the capture is the only external anchor for the Go port.
2. **`go test` never runs C.** The suites are hermetic. Why: a failure always means the Go code drifted, never the environment.
3. **Re-capture only when the pinned C changes.** Go drift fails automatically against the frozen values; C drift is caught by the re-capture check and the differential suite against the reference binary.
...
3. **The capture driver runs it.** `capture.go` compiles the oracle, runs it once per input, and writes the JSON fixture with the input hex and the hash the C engine produced.
4. **The fixture is frozen.** `hash_test.go` loads it and compares the Go port's hashes of the same inputs — equality proves the port byte-identical to the C code on those inputs. Re-running the capture must reproduce the fixture exactly (see upstream sync).
...
- `cc` (gcc or clang) — compiles the oracles (capture only)
- `unzip` — extracts the reference zips once
- Go toolchain 1.25+ — capture and Go suites
...
3. Run `go run ./testdata/capture.go`. If any vector changed, the Go port must be updated to match (differential rule, Q23); the algorithm is not expected to change, but line numbers do, so update the L-number annotations in the port files.
